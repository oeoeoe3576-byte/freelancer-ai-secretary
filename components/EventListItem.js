...
export default function EventListItem({ event, brand, project, showProject = true, onPress }) {
  const color = brand?.color || theme.primary;
  return (
    <TouchableOpacity onPress={onPress} style={styles.row}>
      <View style={[styles.dot, { backgroundColor: color }]} />
      <View style={styles.body}>
        <Text style={[styles.title, event.done && styles.doneText]} numberOfLines={1}>{event.done ? '✓ ' : ''}{event.title}</Text>
